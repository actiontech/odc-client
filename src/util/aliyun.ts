...
export async function uploadFileToOSS(
  ...
  // 1. 获取文档上传坐标
  const uploadMeta = await request.post(
    '/api/v2/cloud/resource/generateTempCredential',
    {
      data: {
        fileName: file.name
      }
  ...
  // 3. 上传文件
  const S3 = await import('aws-sdk/clients/s3').then(
  ...
  async function getResult() {
    const result = await request.get(
      '/api/v2/cloud/specific/getUploadResult/' + uploadId
    );
    if (result?.isError) {
      pullTime = Date.now();
      logger.info(
...
export async function downloadTransferTaskFile(taskId) {
  // 获取下载文件坐标
  const fileInfo = await request.post(
    '/api/v2/cloud/specific/DownloadTransferFile',
    {
      data: {
        taskId: taskId,
