...
function resolveProjectName(
  ...
  if (projectNameHint?.trim()) {
    return projectNameHint.trim();
  }
  if (context.db_service_name?.includes(':')) {
    return getDMSProjectNameByDatasourceName(context.db_service_name) || '';
  }
  return '';
}
...
export const openDMSPrivilegeApply = (
  ...
  const url = generateDMSPrivilegeApplyUrl(context, projectNameHint);
  if (!url) {
    return;
  }
  openDMSUrl(url);
