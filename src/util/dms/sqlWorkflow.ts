...
const createUrlParams = (params: {
  ...
  try {
    ...
    if (params.schema) {
      exportData.schema = params.schema;
    }
...
export const generateDMSSqlWorkflowUrl = (params: Params) => {
  ...
  const baseUrl = `${ROUTE_PATHS.BASE.TRANSIT.index.path}?from=${TRANSIT_FROM_CONSTANT.odc_client}&project_name=${projectName}&to=create_workflow`;
