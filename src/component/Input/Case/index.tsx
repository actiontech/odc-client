...
function onChangeCaseWrap({ caseSensitive = false, escapes = '"' }: Options) {
  ...
  return (
    ...
    if (!oriValue) {
      return {
        displayValue: oriValue,
        onChangeValue: getReturnValue(oriValue)
      };
    }
    ...
    const isWrapped =
      oriValueTrim.startsWith(escapes) && oriValueTrim.endsWith(escapes);
...
const CaseTextArea = forwardRef<TextAreaRef, ICaseTextAreaProps>(
  function CaseTextArea(props: ICaseTextAreaProps, ref) {
    const { value, caseSensitive, escapes, onChange, ...rest } = props;
    const [innerValue, setInnerValue] = useState<string>();
    const [onChangeValue, setOnChangeValue] = useState<any>();
    useEffect(() => {
...
const CaseEditableText: React.FC<CaseEditableTextProps> = React.memo(
  (props) => {
