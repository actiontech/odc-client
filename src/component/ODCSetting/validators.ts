...
export const validForQueryQueryNumber = (value) => {
  const sessionQueryLimit = sessionStorage.getItem(
    `maxQueryLimit-${getCurrentOrganizationId()}`
  );
  const queryLimit = setting.getSpaceConfigByKey(
  ...
  if (sessionQueryLimit) {
    if (value > Number(sessionQueryLimit)) {
      return Promise.reject(
        formatMessage(
          {
            id: 'src.component.ODCSetting.5127321B',
            defaultMessage: '不超过{sessionQueryLimit}'
          },
          { sessionQueryLimit }
        )
      );
  ...
  if (value > Number(queryLimit)) {
    return Promise.reject(
      formatMessage(
        {
          id: 'src.component.ODCSetting.A8E22CC2',
          defaultMessage: '不超过{queryLimit}'
        },
        { queryLimit }
      )
    );
...
export const validForQueryLimit = async (value) => {
  ...
  if (value > maxResultsetRows) {
    return Promise.reject(
      formatMessage(
        {
          id: 'src.component.ODCSetting.9651E837',
          defaultMessage: '不超过{maxResultsetRows}'
        },
        { maxResultsetRows }
      )
    );
