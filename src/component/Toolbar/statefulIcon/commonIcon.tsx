...
export default function commonIcon(wrapIconProps: Partial<any>) {
  const CommonIcon: React.FC<IStatefulIconProps> = (
    props: IStatefulIconProps
  ) => {
    ...
    switch (status) {
      case IConStatus.DISABLE: {
        return (
          <IconComponent className={styles.disable} {...rest} {...iconProps} />
        );
      }
      case IConStatus.RUNNING: {
        return (
          <IconComponent {...rest} className={styles.loading} {...iconProps} />
        );
      }
