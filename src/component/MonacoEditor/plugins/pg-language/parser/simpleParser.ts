...
export class SimpleSQLParser {
  ...
  getCompletionContext(
    ...
    // 检查是否在对象访问符（.）之后
    // PostgreSQL 使用 schema.table.column 格式
    ...
    if (dotMatch) {
      ...
      if (parts.length === 2) {
        ...
        // schema. → 提示表名
        // 或 table. → 提示字段（需要在 objectAccess 中判断）
        return [
          {
            type: 'objectAccess',
            objectName: parts[0]
  ...
  private getCurrentStatement(sql: string, offset: number, delimiter: string) {
    ...
    for (let i = 0; i < sql.length; i++) {
      ...
      // 处理 dollar-quoted strings (PostgreSQL 特有)
      if (!inComment && !inString && !inDollarQuote && char === '$') {
        // 寻找结束的 $
        let tagEnd = i + 1;
        while (tagEnd < sql.length && /[\w$]/.test(sql[tagEnd])) {
          tagEnd++;
        }
