...
import { conf, language } from './monarch/pg';
import { IPGModelOptions } from './service';
import PGAutoComplete from './autoComplete';
import PGHover from './hover';

// PostgreSQL 插件接口，提供与 ob-language 插件类似的 API
export interface IPGPlugin {
...
const LANGUAGE_ID = 'pg';

class PGPlugin implements IPGPlugin {
  private registeredLanguages: string[] = [];
  private autoCompleteProvider: PGAutoComplete;
  private hoverProvider: PGHover;
  private isLanguageRegistered = false;

  constructor() {
    this.autoCompleteProvider = new PGAutoComplete();
    this.hoverProvider = new PGHover();
  }
...
let plugin: PGPlugin | null = null;

export function register(language: string): IPGPlugin {
  language = language || 'pg';
