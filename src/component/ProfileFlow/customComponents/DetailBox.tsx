...
export default ({ dataSource, topNodes, initialNodes, globalInfo }: Iprops) => {
    ...
    subNodeData.percentageInCompare = subNodeData?.overview
      ? (
          (subNodeData?.overview?.[CPU_TIME] /
            (subNodeData?.overview?.[CPU_TIME] +
              subNodeData?.overview?.[IO_WAIT_TIME])) *
          100
        ).toFixed(2)
      : 0;
  }
                        ...
                        {i.value === SUM ? (
                          ...
                          <>
                            ...
                            {sortType === subNodeSortType.BY_DURATION ? (
                              formatTimeTemplate(
                                BigNumber(i?.[sortType]).div(1000000).toNumber()
                              )
                            ) : (
  ...
  function getSubNodesOptions(sortBy = subNodeSortType.BY_DURATION) {
    ...
    const sum = {
      ...
      output:
        data?.statistics?.[
          subNodesSortMap[subNodeSortType.BY_OUTPUT].objectKey
        ],
      maxMemory:
        data?.statistics?.[
          ...
          return (
            ...
            >
              <div className={styles.top5Name}>
                ...
                <div style={{ color: 'var(--profile-text-color)' }}>
                  [{i?.id}]
                </div>
              </div>
              <span className={styles.value}>
                {' '}
                {formatTimeTemplate(
                  BigNumber(i?.data?.duration).div(1000000).toNumber()
                )}{' '}
              </span>
            </div>
          );
      ...
      <div className={styles.customDetailBox}>
        ...
        {globalInfo?.overview && Object.keys(globalInfo?.overview)?.length ? (
          <>
            ...
            <div className={styles.infoBlockBox}>
              <h3
                className={styles.customDetailBoxTitle}
                style={{ paddingTop: 0 }}
              >
                {formatMessage({
                  ...
                  <div className={styles.keyValueBox}>
                    {[IO_WAIT_TIME, CPU_TIME].includes(key) ? (
                      <>
                        ...
                        <span className={styles.value}>
                          {formatTimeTemplate(
                            BigNumber(value).div(1000000).toNumber()
                          )}{' '}
                        </span>
                      </>
                    ) : (
            ...
            Object.keys(globalInfo?.statistics)?.length ? (
              <div className={styles.infoBlockBox}>
                <h3 className={styles.customDetailBoxTitle}>
                  {formatMessage({
