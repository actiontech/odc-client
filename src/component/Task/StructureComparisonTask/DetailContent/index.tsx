...
const CompareTable: React.FC<{
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (operationType: EOperationType) =>
        EOperationTypeMap?.[operationType] || '-'
    },
    {
      title: formatMessage({
  ...
  useEffect(() => {
    if (
      ...
      [SubTaskStatus.DONE, SubTaskStatus.FAILED].includes(
        (currentTaskResult as any)?.status
      )
    ) {
  ...
  return (
    <div style={{ height: '316px', paddingTop: '8px' }}>
      <ConfigProvider
        renderEmpty={() => <TableEmpty description={getDescription()} />}
      >
        <CommonTable
          key="CompareTable"
          ref={tableRef}
  ...
  const getDescription = () => {
    switch (task?.status) {
      ...
      case TaskStatus.EXECUTING: {
        return (
          <span
            style={{ color: 'var(--text-color-placeholder', cursor: 'default' }}
          >
            {formatMessage({
...
const StructureComparisonTaskContent: React.FC<IStructureComparisonTaskContentProps> =
  inject('modalStore')(
    observer((props) => {
      ...
      const loadStructureComparisonResults = async (
        ...
        const data = await getStructrueComparison(
          (currentResult as any)?.taskId,
          {
            ...params
          } as any
        );
