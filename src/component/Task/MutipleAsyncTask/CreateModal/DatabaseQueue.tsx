...
export const DatabaseQueueSelect: React.FC<{
  ...
  ({ rootName, multipleDatabaseChangeOpen, setDefaultDatasource }) => {
    ...
    const orderedDatabaseIds = Form.useWatch(
      ['parameters', 'orderedDatabaseIds'],
      form
    );
    const [databaseIdMap, setDatabaseIdMap] = useState<Map<number, boolean>>(
      new Map()
    );
    ...
    const databaseOptions = useMemo(() => {
      ...
      const selectedDbInfo = _databaseOptions?.find(
        (_db) => _db.value === selectedDbId
      );
      if (selectedDbId) {
        // 这里加同数据源类型的限制
...
const DatabaseQueue: React.FC<{
  ...
  const [selectTemplateModalOpen, setSelectTemplateModalOpen] =
    useState<boolean>(false);
  const [manageTemplateModalOpen, setManageTemplateModalOpen] =
    useState<boolean>(false);
  return (
    <Form.Item
      label={formatMessage({
