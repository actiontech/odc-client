...
const InnerSelect: React.FC<{
  ...
  const [{ canDrop, isOver }, drop] = useDrop({
    ...
    drop: (item, monitor) => {
      if (item?.originOuterName === outerName) {
        ...
        return swapSiblingIndex(
          item?.originOuterName,
          item?.originInnerIndex,
          innerIndex
        );
      } else {
        // target
        ...
        const originOrderedDatabaseIds =
          orderedDatabaseIds?.[item?.originOuterName];
        originOrderedDatabaseIds?.splice(item?.originInnerIndex, 1);
        orderedDatabaseIds?.splice(
          item?.originOuterName,
          1,
          originOrderedDatabaseIds
        );
        const newOrderedDatabaseIds = orderedDatabaseIds?.filter(
          (ids) => ids?.length
        );
  ...
  const [{ opacity, isDragging }, drag, preview] = useDrag({
    item: {
      originInnerIndex: innerIndex,
      originOuterName: outerName,
      type: 'box'
    },
    collect: (monitor) => ({
  ...
  const renderItem = (
    ...
    const isSelected =
      databaseIds?.includes(item?.value) &&
      item.value !== currnetOrderedDatabaseId;
                ...
                renderOptions?.length ? (
                  ...
                  >
                    {renderOptions?.map((item) =>
                      renderItem(item, setSearchValue)
                    )}
                  </div>
                ) : (
                  <DatabaseSelectEmpty />
...
const InnerSelecter: React.FC<{
  ...
  const [{ canDrop, isOver }, drop] = useDrop({
    ...
    drop: (item, monitor) => {
      ...
      orderedDatabaseIds?.[item?.originOuterName]?.splice(
        item?.originInnerIndex,
        1
      );
      const newOrderedDatabaseIds = orderedDatabaseIds?.filter(
  ...
  return (
    <div>
      {innerFields?.map(
        ({ key: innerKey, name: innerName, ...innerRestField }, innerIndex) => (
          <InnerSelect
            rootName={rootName}
            key={innerIndex}
            disabled={disabled}
