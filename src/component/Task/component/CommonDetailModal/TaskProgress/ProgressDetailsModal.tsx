...
export default ({ modalOpen, handleClose, parametersJson, resultJson }) => {
  ...
  const {
    ...
    fullTransferProgressPercentage,
    currentStep
  } = resultJson || {};
  ...
  const statusList = [
    ...
    },
    {
      ...
      value:
        ProgressOfLocklessStructureChangeTaskStatusMap.MONITOR_DATA_TASK_TRANSFER_PRECHECK
    },
    {
      title: formatMessage({
      ...
      value:
        ProgressOfLocklessStructureChangeTaskStatusMap.MONITOR_DATA_TASK_FULL_TRANSFER
    },
    {
      title: formatMessage({
  ...
  useEffect(() => {
    const tempStepsItems = statusList.map((item) => {
      ...
      if (
        item.value === 'MONITOR_DATA_TASK_FULL_TRANSFER' &&
        fullTransferProgressPercentage
      ) {
        return {
          ...item,
