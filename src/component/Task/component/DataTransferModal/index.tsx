...
class TaskContent extends React.Component<any, any> {
  ...
  renderExt = (isImport: boolean) => {
    ...
    const isExpired =
      Math.abs(Date.now() - task?.completeTime) >= 14 * 24 * 60 * 60 * 1000 ||
      false;
    return (
      <>
        <Row>
                    ...
                    return (
                      ...
                      >
                        <a
                          key={index}
                          style={{
                            cursor: isExpired ? 'not-allowed' : undefined
                          }}
                          type="link"
                          onClick={() => {
                            if (isExpired) {
          ...
          {task?.executionStrategy === TaskExecStrategy.TIMER && (
            <Col span={12}>
              <SimpleTextItem
                ...
                /*执行时间*/ content={getLocalFormatDateTime(
                  task?.executionTime
                )}
              />
            </Col>
