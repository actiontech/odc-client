...
const checkIsProNameOrDBName = (expression: Expression): boolean => {
  return [Expression.PROJECT_NAME, Expression.DATABASE_NAME].includes(
    expression
  );
};
const Condition = ({
  ...
  const [condition, setCondition] = useState<any>(
    parentIsRoot
      ...
      : formRef.getFieldsValue()?.conditions?.[indexChan?.[0]]?.children?.[
          indexChan?.[1]
        ]
  );
  ...
  const [valueMap, setValueMap] =
    useState<{ [key in string | number]: string }>(environmentMap);
        ...
        <Select
          ...
          onSelect={(_, { value }) => {
            ...
            if (parentIsRoot) {
              ...
              data.conditions[indexChan[0]].children[indexChan[1]].expression =
                value;
              data.conditions[indexChan[0]].children[indexChan[1]].value =
                undefined;
              formRef.setFieldsValue({
                ...data
              });
