...
class UserPage extends React.PureComponent<IProps, IState> {
  ...
  private getPageColumns = (roles: any[]) => {
    return [
      ...
      {
        title: formatMessage({
          id: 'odc.components.UserPage.Role',
          defaultMessage: '角色'
        }), // 角色
        dataIndex: 'roles',
        ellipsis: true,
...

      {
        ...
        render: (lastLoginTime) =>
          lastLoginTime ? getFormatDateTime(lastLoginTime) : '-'
      },

      {
        title: formatMessage({
          id: 'odc.Auth.User.EnableStatus',
          defaultMessage: '启用状态'
        }), //启用状态
        width: 80,
        ellipsis: true,
      ...
      {
        title: formatMessage({
          id: 'odc.components.UserPage.Operation',
          defaultMessage: '操作'
        }), // 操作
        width: 132,
        key: 'action',
  ...
  private handleUserEnable = async (data: {
    user: IManagerUser;
    enabled: boolean;
  }) => {
    ...
    if (res) {
      message.success(
        enabled
          ? formatMessage({
              id: 'odc.components.UserPage.Enabled',
              defaultMessage: '启用成功'
            }) // 启用成功
          : formatMessage({
              id: 'odc.components.UserPage.Disabled',
              defaultMessage: '停用成功'
            }) // 停用成功
      );
      this.reloadData();
      ...
      message.error(
        enabled
          ...
          : formatMessage({
              id: 'odc.components.UserPage.Disabled.1',
              defaultMessage: '停用失败'
            }) // 停用失败
      );
    }
