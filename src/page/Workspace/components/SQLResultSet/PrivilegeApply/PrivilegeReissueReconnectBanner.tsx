...
function readTracking(): TrackingItem[] {
  try {
    const raw = localStorage.getItem(PRIVILEGE_REISSUE_TRACKING_KEY);
...
function writeTracking(items: TrackingItem[]) {
  localStorage.setItem(PRIVILEGE_REISSUE_TRACKING_KEY, JSON.stringify(items));
}
...
const PrivilegeReissueReconnectBanner: React.FC<IProps> = ({ context }) => {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => {
      if (event.key === PRIVILEGE_REISSUE_TRACKING_KEY) {
        checkReissueStatus();
      }
    };
