...
export const getMessageColumns: GetMessageColumns = function ({
  ...
  return [
    ...
    },
    {
      ...
      render: (createTime) =>
        createTime ? getLocalFormatDateTime(createTime) : '-'
    },
    {
      title: formatMessage({
      ...
      sortDirections: ['ascend', 'descend'],
      render: (lastSentTime) =>
        lastSentTime ? getLocalFormatDateTime(lastSentTime) : '-'
    },
    {
      title: formatMessage({
...
export const getPolicyColumns: GetPolicyColumns = function ({
            ...
            if (index < channelNames?.length - 1) {
              return (
                <>
                  <a
                    key={index}
                    onClick={() => hanleOpenChannelDetailDrawer(channel)}
                  >
                    {channel?.name}
                  </a>
                  、
  ...
  return columns.filter(
    (item) => !hideColumns.includes(item.key as IPolicyColumnsKeys)
  );
};
// #endregion
...
export const getChannelColumns: GetChannelColumn = function ({
  ...
  return columns.filter(
    (item) => !hideColumns.includes(item.key as IChannelColumnsKeys)
  );
};
// #endregion
